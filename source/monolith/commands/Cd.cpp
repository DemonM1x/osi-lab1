...
void Cd::Run(const std::vector<std::string>& args) {
  ...
  try {
    if (new_directory == "..") {
      console_->previous_directories_.push(current_path.string());  // Сохранение текущего пути
    ...
    console_->UpdateWorkingDirectory(std::filesystem::current_path().string()); // Обновление пути
...
void Console::UpdateWorkingDirectory(const std::string& new_directory) {
  working_directory_ = new_directory;  // Update the working directory member variable
  std::cout << "Working directory updated to: " << working_directory_ << std::endl; // Optional: Inform the user
}
